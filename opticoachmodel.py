...
class OpticoachModel:
    ...
    def build_with_hyperparams(self, hp: HyperParameters):
        '''
        ...
        print(f"shape(numericalConcatenation) = {shape(numericalConcatenation)}")
        ...
        print(f"shape(maskedInputs) = {shape(maskedInputs)}")
        ...
        lstmLayer = LSTM(
            ...
            return_sequences=False  # Output a single vector for the entire sequence
        )(maskedInputs)
        print(f"shape(lstmLayer) = {shape(lstmLayer)}")

        # Repeat the LSTM output for 3 years to match the desired output sequence length
        repeatedLayer = RepeatVector(3)(lstmLayer)
        print(f"shape(repeatedLayer) = {shape(repeatedLayer)}")
        ...
        print(f"shape(hiddenLayer) = {shape(hiddenLayer)}")
    ...
    def build_and_train_directly(self):
        '''
        Build the recurrent neural network.
        https://www.tensorflow.org/guide/keras/working_with_rnns
        https://keras.io/api/layers/recurrent_layers/gru/
